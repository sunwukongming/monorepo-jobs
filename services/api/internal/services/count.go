...
func CountUpdatePassageRecommendByPath(path string) error {
	...
	var err error
	// 更新职位推荐数量
	err = db.Default().Exec(`update passage_recommend as pr set
...
func CountUpdatePassageRecommend(passageRecommendId int) error {
	...
	err = db.Default().Exec(`update passage_recommend as pr set
		...
		share_count = (select count(*) from deliver where passage_id = pr.passage_id and recommend_account_id = pr.account_id and recommend_account_id != account_id),
		share_count_l2 = (select count(*) from deliver where passage_id = pr.passage_id and passage_recommend_path = concat(pr.path, '-', pr.id))
		where id = ?`, passageRecommendId).Error
	if err != nil {
		logrus.WithFields(logrus.Fields{
			"action":  "counter_update",
			"message": err.Error(),
...
func CountUpdateAccount(accountId int) error {
	var err error
	//更新用户自荐数量
	err = db.Default().Exec("update account as a set self_recommend = (select count(*) from deliver where account_id = a.id and is_real = 1) where id = ?", accountId).Error
	if err != nil {
		logrus.WithFields(logrus.Fields{
			"action":  "counter_update",
			"message": err.Error(),
	...
	// 更新用户推荐数量
	err = db.Default().Exec("update account as a set recommend = (select count(*) from deliver where recommend_account_id = a.id and is_real = 1 and recommend_account_id != account_id) where id = ?", accountId).Error
	if err != nil {
		logrus.WithFields(logrus.Fields{
			"action":  "counter_update",
			"message": err.Error(),
